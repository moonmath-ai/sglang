...
"""Abstract interface for fuzzy (position-independent) prefix matching providers."""
...
class FuzzyMatchResult(msgspec.Struct):
    ...
    # Optional fields reserved for segmented (N:M) providers.
...
def create_fuzzy_match_provider(
    ...
    logger.info(f"Creating fuzzy match provider: {provider_name}")

    if provider_name == "ExactHash":
        ...
        raise ValueError(
            ...
            f"Supported providers: 'ExactHash'"
        )
