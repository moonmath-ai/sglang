...
class DeepseekV4HipRadixBackend(
    ...
    def __init__(
        ...
        # DSpark verify metadata is assembled entirely on-device. Advertising the
        # inherited default (True) forces FutureMap to publish a CPU mirror every
        # cycle and defeats the overlap scheduler's GPU-only relay.
        self.needs_cpu_seq_lens = not model_runner.spec_algorithm.is_dspark()
    ...
    def init_forward_metadata_prefill(
        ...
        batch_indices_repeated = _expanded.batch_indices_repeated
    ...
    def init_forward_metadata_target_verify_old(
        ...
        if ragged_layout is not None:
            ...
            if seq_lens_cpu is None:
                ...
                extend_start_loc = torch.arange(
                    batch_size, dtype=torch.int32, device=seq_lens.device
                ) * self.target_verify_num_draft_tokens
                extend_seq_lens_cpu = None
            else:
                seq_lens_cpu = [
        ...
        return self.init_forward_metadata_prefill(
            ...
            need_compress=True,
            use_prefill_cuda_graph=use_prefill_cuda_graph,
            compress_gpu_plan=extend_start_loc is not None,
            extend_start_loc=extend_start_loc,
    ...
    def init_forward_metadata_out_graph(
        ...
        if seq_lens_cpu is not None:
            ...
            torch._assert_async((seq_lens <= chosen_max_seq_len).all())
    ...
    def _attach_unified_kv_prefill_meta(
        ...
        extend_seq_lens: torch.Tensor,
        num_tokens: int,
        batch_indices_repeated: Optional[torch.Tensor] = None,
        exact_num_tokens: bool = True,
    ) -> None:
        from sglang.kernels.ops.attention.dsv4.unified_kv_kernels.env_gate import (
            is_unified_kv_triton,
        ...
        # Reuse the expansion only when the caller proves the live token count.
        # Keep the existing fallback for ragged tiers and EAGLE synchronization.
        if batch_indices_repeated is not None and exact_num_tokens:
            # Expansion already includes graph padding; padding below adds its
            # own sentinel values, so consume only the live token rows here.
            bid = batch_indices_repeated[:num_tokens]
        else:
