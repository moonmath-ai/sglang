...
class _RecordingAllocator:
    ...
    def free_segment(self, free_index: torch.Tensor, *, start_pos: int) -> None:
        # Current allocator protocol (allocator/base.py): free_segment /
        # free_segments wrap plain per-slot free(); page-boundary dedup is a
        # real-allocator concern, not something these slot-count tests model.
...
class TestFuzzyMatchGating(CustomTestCase):
    ...
    def test_key_limit_reservation_clamps_an_overreaching_fuzzy_match(self):
        """(bug regression) ``RadixKey.limit`` reserves the prompt's last
        token so the extend forward pass always has >=1 token to actually
        run (``Req._compute_max_prefix_len``). Providers see the *un*trimmed
        tail (so their own chunk boundaries line up with the registration
        side), so a provider can legitimately return a match reaching the
        tail's true end; ``_apply_fuzzy_result`` must clamp it back down to
        ``total_len`` (which already reflects ``key.limit``). Skipping this
        clamp let a fuzzy match consume every token including the reserved
        one whenever a match reached that far -- observed as a real KL-
        divergence blowup (0.08 vs. a 0.02 threshold) once a provider could
        combine multiple chunks through the tail's end; a provider that only
        ever returns one small leading chunk essentially never reached that
        far, so this was unexercised until then."""
        scripted = _scripted_fuzzy_result(
            cached_token_count=5,
            kv_indices=[50, 51, 52, 53, 54],
            cached_start_pos=10,
        )
        cache, provider, _allocator = _make_cache(scripted_result=scripted)
        req = _StubReq(rid="limited")

        key = RadixKey(token_ids=array("q", [1, 2, 3, 4, 5]), limit=4)
        result = cache.match_prefix(MatchPrefixParams(key=key, req=req))

        self.assertEqual(provider.match_calls[0]["prompt_token_ids"], [1, 2, 3, 4, 5])
        self.assertEqual(result.fuzzy_matched_len, 4)
        self.assertEqual(result.device_indices.tolist(), [50, 51, 52, 53])
...
class TestDonorLifecycle(CustomTestCase):
    ...
    def test_realization_narrows_protected_prefix_to_exact(self):
        ...
        realizer.is_mla_pool = False
        realizer.device = "cpu"
        realizer.rotary_emb = MagicMock()
        realizer.naive_reuse_hooks = None
