...
already-running server:
    ...
    python test/manual/fuzzy_match/measure_conditional_ttft_mla.py <base_url> [donor_tokens]

`donor_tokens` (optional, default 150) sets the realized span size — e.g.
`... http://127.0.0.1:21000 700` reproduces the >512-token ceiling probe
from POC_PLAN.md's "Multi-chunk realization" without hand-editing a copy of
this file. Below chunker.MAX_CHUNK_TOKENS (512), a smaller value tends to
land within a single CDC chunk more often; above it, multi-chunk extension
is required to realize the whole span (see POC_PLAN.md's "Multi-chunk
realization" for why that used to cap the realized span at 512 regardless
of donor_tokens, before the multi-chunk fix).
...
SINK_TOKENS = 32
DONOR_TOKENS = int(sys.argv[2]) if len(sys.argv) > 2 else 150
